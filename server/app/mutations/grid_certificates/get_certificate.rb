...
module GridCertificates
  class GetCertificate < Mutations::Command
    ...
    def execute
      ...
      upsert_secret(self.secret_name, [cert, cert_priv_key].join)
    ...
    def upsert_secret(name, value)
      cert_secret = self.grid.grid_secrets.find_by(name: self.secret_name)
      if cert_secret
        outcome = GridSecrets::Update.run(grid_secret: cert_secret, value: value)
      else
        outcome = GridSecrets::Create.run(grid: self.grid, name: self.secret_name, value: value)
      end
